...
重申一次，如果你使用字符串模板或单组件文件，那么这个限制就不存在了。
...
可以使用 v-bind 来告诉 Vue , 你传入的 prop 是一个 Javascript 表达式而不是一个简单的字符串。
...
这种声明方式只是以字符串数组的形式，简单的声明了该组件接收的来自父组件的变量名称。



### 2) 对象形式声明 props

如果在子组件中希望能够指定接收的值类型，可以采用对象的形式来声明 props 。
...
```
非 Prop 的 Attribute 会被自动添加到这个组件的根元素上 (如果这个组件中声明了 inheritAttrs: false的话)
并且保存在这个组件的 $attrs 属性中 (不管是否有 inheritAttrs: false)
```

这也是为什么组件可以接受任意的 attribute，而这些 attribute 会被添加到这个组件的根元素上（未声明 **inheritAttrs:false** 的话 ）。
...
额外的，每次父级组件发生变更时，子组件中所有的 prop 都将会刷新为最新的值。这意味着你**不**应该在一个子组件内部改变 prop。
...
<font color="red">注意在 JavaScript 中对象和数组是通过引用传入的，所以对于一个数组或对象类型的 prop 来说，在子组件中改变变更这个对象或数组本身**将会**影响到父组件的状态。</font>
