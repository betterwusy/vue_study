...
为了让 `user` 在父级的插槽内容中可用，我们可以将 `user` 作为 `<slot>` 元素的一个 attribute 通过  `v-bind:`  绑定上去：
...
> v-slot 与 slot 的主要区别：
>
> ​	1）v-slot:slotName="slotProps" 等同于 slot="slotName" slot-scope="slotProps"
>
> ​	2）v-slot 必须用在 template 上(独占默认插槽可以直接在组件上 v-slot="slotProps")  ,
>
> ​		  slot 和 slot-scope 则不要求一定要用在 template 上，可以用在任意标签上
...
> v-slot 与 slot 的主要区别：
>
> ​	1）v-slot 的语法是 v-slot:slotName , 而 slot 的语法是 slot="slotName"
>
> ​	2）v-slot 必须用在 template 上(独占默认插槽可以直接在组件上 v-slot="slotProps")  ,
>
> ​		  slot 则不要求一定要用在 template 上，可以用在任意标签上
...
>
> v-slot 与 slot-scope 的区别：
>
>  1）v-slot 标签将插槽Props放在标签的表达式中，v-slot:slotName="slotProps"  ===  slot-scope="slotProps"
>
>  2）同 slot 的用法一样，slot-scope 也可以用于非 template 标签上
