...
```html
<div id="example-2">
  <!-- 使用 @click="greet" 的形式调用方法，
		事件处理方法会接收一个DOM event 对象 -->
...
- `.stop`	阻止事件继续传播（阻止冒泡）
- `.prevent`  阻止事件的默认行为，如：点击超链接自动页面跳转、点击表单按钮自动重载页面等
...
>
> 请记住，`.passive` 会告诉浏览器你*不*想阻止事件的默认行为。
...
比如： `@keyup.ctrl.enter` 需要按住 ctrl 并释放 enter 按键时，才会触发事件。而单单释放 `ctrl` 也不会触发事件。
