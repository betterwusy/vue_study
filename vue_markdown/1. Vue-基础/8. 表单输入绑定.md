...
<template v-for="item in options">
  <input type="checkbox" :id="item" :value="item" v-model="checkedNames"></input>
...
在默认情况下，`v-model` 在每次 `input` 事件触发后将输入框的值与数据进行同步 (除了[上述](https://cn.vuejs.org/v2/guide/forms.html#vmodel-ime-tip)输入法组合文字时)。
