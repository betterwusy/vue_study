...
class attribute 接受字符串、对象、字符串或对象组成的数组 
...
上面的语法表示<font color="#d63200;"> `active` </font> 这个 class 存在与否将取决于数据 property <font color="#d63200;"> `isActive` </font>的 [truthiness](https://developer.mozilla.org/zh-CN/docs/Glossary/Truthy)。

你可以在对象中传入更多字段来动态切换多个 class。此外，<font color="#d63200;">`v-bind:class` </font> 指令也可以与普通的 class attribute 共存。当有如下模板：
...
这样写只会渲染**数组中最后一个被浏览器支持的值**。在本例中，如果浏览器支持不带浏览器前缀的 flexbox，那么就只会渲染 `display: flex`。
