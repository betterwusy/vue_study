...
![image-20211027091556903](2. 模板语法.assets/image-20211027091556903.png)
...
动态参数预期会求出一个字符串，异常情况下值为 `null`。这个特殊的 `null` 值可以**被显性地用于移除绑定**。任何其它非字符串类型的值都将会触发一个警告。
...
变通的办法是使用没有空格或引号的表达式，或**用计算属性替代这种复杂表达式**。
